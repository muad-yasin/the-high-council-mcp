...
The bundle is not in the npm package; it is built from it.
...
| One key per provider (all optional, empty by default) | `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, ... | Marked `sensitive`, so the app keeps them in the OS keychain. There is no default key and none is ever shipped. The `mock` chains need no key. |
