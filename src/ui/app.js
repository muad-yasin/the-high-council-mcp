...
async function main() {
  ...
  const sel = $('#runs');
  sel.innerHTML = runs.map(r => `<option value="${esc(r.id)}">${esc(r.id)}  ${esc(r.chain)}  ${esc(r.task.replace(/^tasks\//, ''))}  ${r.finished ? (r.passed ? 'signed off' : 'open') : 'unfinished'}  ${r.words ? r.words + 'w' : ''}  ${usd(r.usd)}</option>`).join('');
...
function renderOutline() {
  ...
  const opts = run.drafts.map(x => `<option value="${esc(x.label)}" ${x === d ? 'selected' : ''}>${esc(x.label)} (${x.words} words)</option>`).join('');
  box.innerHTML = `<div class="small">Draft: <select id="draftpick">${opts}</select> - ${d.words} words, ${flat.length} sections.
    ...
    flat.map((s, i) => `<div class="row l${s.level}" data-i="${i}">
      <span class="t" title="${esc(s.path)}">${esc(s.title)}</span>
      <span class="n">${s.words}</span>
      <span><div class="bar"><i style="width:${(100 * s.words / maxW).toFixed(1)}%"></i></div><div class="bar vol" style="margin-top:2px"><i style="width:${(100 * s.volume.score / maxV).toFixed(1)}%"></i></div></span>
    </div>`).join('');
  ...
  box.querySelectorAll('.row[data-i]').forEach(r => r.onclick = () => {
    ...
    const v = s.volume;
    $('#reader').innerHTML = `<div class="small" style="white-space:normal;font-family:system-ui">${esc(s.path)} - ${s.words} words (${s.ownWords} own)
      <br>volume ${v.score}: ${v.files.length} file(s), ${v.scripts.length} script name(s), ${v.codeBlocks} code block(s), ${v.tableRows} table row(s), ${v.checklistItems} checklist item(s), ${v.numbers} unit-carrying number(s)
...
function renderRevisions() {
  ...
  const rows = run.comparison.map(c => {
    const cells = labels.map((l, i) => {
      ...
      if (!prev) return `<td class="n ${i ? 'new' : ''}">${cur.words}${i ? ' new' : ''}</td>`;
      const d = cur.words - prev.words;
      return `<td class="n">${cur.words} <span class="${d > 0 ? 'up' : d < 0 ? 'down' : 'small'}">${d > 0 ? '+' : ''}${d || ''}</span></td>`;
    }).join('');
    return `<tr><td class="l${c.level}" style="padding-left:${8 + (c.level - 1) * 14}px">${esc(c.title)}</td>${cells}</tr>`;
  }).join('');
  const totals = `<tr><th>whole draft</th>${run.drafts.map(d => `<th class="n">${d.words}</th>`).join('')}</tr>`;
  const rounds = Object.keys(run.panels).sort((a, b) => a - b).map(r => {
    ...
    const drove = run.drafts.some(d => d.label === `revise-${r}`) ? `drove revise-${r}` : 'left open (round cap reached, no further revision)';
    return `<details class="card"><summary>Round ${r}: ${items.length} objection(s) ${drove}</summary><ul>${items.join('') || '<li class="small">none</li>'}</ul></details>`;
  }).join('');
...
function renderProposals() {
  ...
  const sb = `<table><thead><tr><th>lab</th><th>model</th><th class="n">proposed</th><th class="n">accepted</th><th class="n">cut</th><th class="n">unaccounted</th><th class="n">built</th></tr></thead><tbody>` +
    score.map(l => `<tr><td>${esc(l.lab)}</td><td class="small">${esc(l.model)}</td><td class="n">${l.proposed}</td><td class="n">${l.accepted}</td><td class="n">${l.cut}</td><td class="n">${l.unaccounted}</td><td class="n small">${l.built ?? 'after the build'}</td></tr>`).join('') + '</tbody></table>';
  const cards = Object.entries(byLab).map(([lab, list]) => `<div class="card"><h3>${esc(lab)} <span class="small">${esc(list[0].model)}</span></h3>` +
    list.map(p => { const l = ledger[p.id]; const st = l?.status || 'unaccounted';
      return `<details><summary><b>${esc(p.id)}</b> ${esc(p.title)} <span class="tag ${st}">${st}</span> <span class="small">${esc(l?.note || '')}</span></summary>
...
function renderPanel() {
  ...
  box.innerHTML = (crit.length ? `<div class="card"><h3>Acceptance criteria</h3><ol>${crit.map(c => `<li>${esc(c)}</li>`).join('')}</ol></div>` : '') +
    rounds.map(r => `<div class="card"><h3>Round ${r}</h3>` + run.panels[r].map(p => {
      ...
      return `<details><summary><b>${esc(p.lab)}</b> <span class="tag ${fails.length ? 'bad' : 'ok'}">${fails.length ? fails.length + ' objection(s)' : 'signed off'}</span> <span class="small">${esc(p.parsed.verdict_line || '')}</span></summary>
...
function renderCost() {
  ...
  const byProv = {};
  for (const s of st) { const k = `${s.provider}/${s.model}`; byProv[k] = (byProv[k] || 0) + (s.usd || 0); }
  box.innerHTML = `<div class="grid2"><div class="card"><h3>By seat</h3><table>${Object.entries(byProv).sort((a, b) => b[1] - a[1]).map(([k, v]) => `<tr><td>${esc(k)}</td><td class="n">${usd(v)}</td></tr>`).join('')}<tr><th>total</th><th class="n">${usd(run.report.totals.usd)}</th></tr></table></div>
    <div class="card"><h3>By stage</h3><table><thead><tr><th>stage</th><th>seat</th><th class="n">in</th><th class="n">out</th><th class="n">thinking</th><th class="n">usd</th><th class="n">s</th></tr></thead><tbody>${st.map(s => `<tr><td>${esc(s.label)}</td><td class="small">${esc(s.provider)}/${esc(s.model)}</td><td class="n">${s.usage?.input ?? ''}</td><td class="n">${s.usage?.output ?? ''}</td><td class="n">${s.usage?.thinking || ''}</td><td class="n">${usd(s.usd)}</td><td class="n">${s.ms ? (s.ms / 1000).toFixed(0) : ''}</td></tr>`).join('')}</tbody></table></div></div>`;
}
...
function renderFiles() {
  ...
  list.innerHTML = names.map(n => `<div class="row" data-f="${esc(n)}"><span class="t">${esc(n)}</span><span class="n">${(run.texts[n] || '').length}</span><span></span></div>`).join('');
