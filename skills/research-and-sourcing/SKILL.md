...
description: Gathering facts an agent will act on or publish, with graded confidence - grading sources rather than just finding one, tracing load-bearing claims to the primary document, telling independent sources from syndicated copies, marking every claim documented or inferred, keeping numbers attached to their setup, capturing verbatim particulars, recency for anything that moves, and taking ideas rather than text from sources. Use before any claim about external reality - library or API behavior, prices, dates, versions, competitors, research findings, statistics, real people or organizations - and whenever a claim "sounds right" but hasn't been traced. Not for checking whether the agent's own output is correct (verification-and-critique), not for deciding what to build (task-scoping), and not for prose style.
---
...
1. **Grade the source, not just the match.** A primary document, an official changelog, a peer-reviewed paper, a first-party announcement, a reputable secondary report, a vendor blog, and an SEO content page are different tiers. One multi-agent research system was observed preferring content farms over authoritative primary sources until explicit source-quality heuristics were added. Say which tier each claim rests on.
...
6. **The summary is not the document.** A search snippet or an AI-generated summary gives the feeling of having read the source while omitting most of it, and the gaps are where invention goes. Tell: the invented version is an absolute ("every," "never," "nobody"). Open the document.
