---
name: task-scoping
description: Turning a request into a bounded, checkable piece of work before acting - acceptance criteria and a stop condition written first, only the clarifying questions that change the outcome, explicit non-goals, risky and irreversible actions flagged up front, work ordered by risk rather than ease, effort sized to the task, and the review checkpoint after something is reported finished. Use at the start of any multi-step or ambiguous task, before delegating work to another agent or session, whenever a new idea arrives mid-task, when scope feels fuzzy, and after anything is reported "done" - even when the request never says "plan." Not for gathering outside facts (research-and-sourcing), proving a result correct (verification-and-critique), handing work between agents (context-and-handoff), or how to operate a tool safely (tool-and-action-discipline).
---

# Task Scoping

Before the first action that changes anything, turn the request into work that has a definition of done. Most agent failures that look like execution failures started here: a constraint dropped between reading and doing, a stop condition nobody wrote, an assumption taken silently, a risky step discovered only when reached. The named taxonomy for multi-agent failure puts "disobeying the task specification" and "not knowing when to stop" among its most common specification-level failures.
...
- **Size effort to the task, explicitly.** Name how many steps, tool calls, or parallel agents the task deserves. Orchestrators without written effort tiers have been observed over-investing heavily in simple requests. More agents cost many times the tokens of one; use them where work is genuinely parallel or independence genuinely matters.
