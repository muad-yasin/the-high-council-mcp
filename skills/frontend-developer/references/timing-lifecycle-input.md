...
- **A disabled control can erase an explanation.** If the only route to a rejection reason is an action the disabled state already blocks, that reason can never be shown. Fine if the state is truly unreachable; a silent information loss if another path can still hit the same rejection. Check explicitly.
- **Locking a surface by disabling it reads as breakage.** A feed that picked up the shared "disabled" tint during a busy state was reported as "it turned grey, unwanted." Keeping it live and letting existing navigation-cancel semantics resolve the conflict needed no new code.
