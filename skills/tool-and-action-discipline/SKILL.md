...
description: Uses tools reliably and safely, and designs tools agents can use reliably - destructive operations only against an enumerated, confirmed target list, budgets enforced before the call, retries for transport failures but never for wrong answers, tool and model output treated as untrusted input, human gates never skipped or faked, permission denials respected, and narrow tools with actionable errors. Use before any action that changes state, spends money or is visible to others; when an action is destructive or hard to reverse; when handling tool errors or retries; and when designing, wrapping or reviewing a tool or MCP server. Not for judging results (verification-and-critique) or deciding what to build (task-scoping).
license: MIT
---

# Tool and Action Discipline

Tools are where an agent's plan touches the world. Two things go wrong there: the agent uses a tool in a way that does damage or wastes budget, and the tool itself is shaped so that misuse is easy. Both halves matter: in the SWE-agent study (2024), changing only the interface - same model - raised the SWE-bench Lite resolve rate from 11% to 18%.
...
- **A few narrow, purpose-shaped tools beat a raw shell or a wrapper around every endpoint.** Each tool takes a small, named argument shape and does one job, and its name says which system it touches (a namespace prefix when several servers are loaded). There should be no path by which a caller can name an arbitrary command.
